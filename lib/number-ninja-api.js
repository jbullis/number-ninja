...
function coinRewardAmount(body, rec, localDate) {
  const reason = String(body.reason || "");
  if (reason === "practice_correct") return { reason, coins: 3, key: null };
  if (reason === "practice_hint" || reason === "practice_retry" || reason === "practice_variant") return { reason, coins: 1, key: null };
  if (reason === "daily_bonus") return { reason, coins: 10, key: "daily_bonus:" + localDate };
  if (reason === "story_level_complete") {
    const levelKey = String(body.levelKey || "").replace(/[^a-zA-Z0-9:_-]/g, "").slice(0, 40);
    if (!levelKey) return { error: "level_key_required" };
    const story = rec && rec.data && rec.data.progress && rec.data.progress.story && rec.data.progress.story[levelKey];
    if (!story || !Number(story.stars || 0)) return { error: "progression_evidence_required" };
    const stars = Math.max(1, Math.min(3, Math.floor(Number(story.stars || 1))));
    return { reason, coins: 30 + 10 * stars, key: "story_level_complete:" + levelKey, stars };
  }
  if (reason === "boss_clear" || reason === "level_up") return { error: "progression_evidence_required" };
...
function awardCoins(rec, body, localDate) {
  ...
  const reward = coinRewardAmount(body, rec, localDate);
  ...
  if (!reward.key && currency.recentEventIds.includes(eventId)) return { duplicate: true, reason: reward.reason, coins: Number(rec.data.coins || 0), awarded: 0 };
