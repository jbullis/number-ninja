...
function dailyPayload(rec, localDate) {
  ...
  return { dailyStatus, recommendations, masterySummary, assignments };
}
...
function assignmentStatusPayload(rec, localDate) {
  ...
  return { assignments: extras.assignments, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls, { localDate }), dailyStatus: extras.dailyStatus, controls: publicControls(rec.controls) };
}
...
async function playerPost(request, env) {
  ...
  if (action === "report" || action === "save" || action === "placement_start" || action === "placement_progress" || action === "placement_complete" || action === "placement_skip" ||
    ...
    if (action === "report") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, data: rec.data || null, controls: publicControls(rec.controls || defaultControls("4")), masterySummary: extras.masterySummary, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, assignments: extras.assignments, updated: rec.updated });
    }
    ...
    if (action === "daily_status") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, masterySummary: extras.masterySummary, assignments: extras.assignments, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "daily_problem_complete") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, duplicate: !!result.duplicate, dailyStatus: result.status, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls), assignments: Assignments.activeSummaries(rec.controls.assignments, { localDate: effective.date, includeNotes: false }), controls: publicControls(rec.controls) });
    }
    ...
    if (action === "assignment_active_time") {
      ...
      const payload = assignmentStatusPayload(rec, effective.date);
      await putRecord(store, name, rec);
      return json({ ok: true, duplicate: !!result.duplicate, completed: !!result.completed, rewardCoins: paid, progress: result.progress, assignment: Assignments.summarizeAssignment(rec.controls.assignments[found.idx], { localDate: effective.date, includeNotes: false }), assignments: payload.assignments, recommendations: payload.recommendations, dailyStatus: payload.dailyStatus, data: rec.data || null, controls: publicControls(rec.controls) });
    }
