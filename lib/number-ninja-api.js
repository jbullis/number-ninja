...
function coinRewardAmount(body) {
  const reason = String(body.reason || "");
  const boosted = body.boosted === true;
  const mult = boosted ? 2 : 1;
  if (reason === "practice_correct") {
    const tier = Math.max(1, Math.min(5, Math.floor(Number(body.tier || 1))));
    return { reason, coins: (2 + tier) * mult };
  }
  if (reason === "practice_hint" || reason === "practice_retry" || reason === "practice_variant") return { reason, coins: 1 * mult };
  if (reason === "daily_bonus") return { reason, coins: 10 };
  if (reason === "boss_clear") return { reason, coins: 25 * mult };
  if (reason === "level_up") return { reason, coins: 20 };
  if (reason === "story_level_complete") {
    const stars = Math.max(1, Math.min(3, Math.floor(Number(body.stars || 1))));
    return { reason, coins: (30 + 10 * stars) * mult };
  }
...
function awardCoins(rec, body, localDate) {
  ...
  if (currency.recentEventIds.includes(eventId)) return { duplicate: true, coins: Number(rec.data.coins || 0), awarded: 0 };
  const reward = coinRewardAmount(body);
