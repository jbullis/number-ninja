...
function summarizeDaily(activity, range, attempts) {
  const byDate = {};
  attempts.forEach((a) => {
  ...
  Object.keys(activity.daily || {}).forEach((date) => {
    ...
    const d = byDate[date] = byDate[date] || { date, attempts: 0, correct: 0, incorrect: 0, help: 0, activeSeconds: 0, skills: [] };
    d.activeSeconds = Number(src.activeSeconds || d.activeSeconds || 0);
    if (Array.isArray(src.skills)) src.skills.forEach((id) => { if (!d.skills.includes(id)) d.skills.push(id); });
  });
...
function buildProgressReport(rec, activity, body) {
  ...
  const daily = summarizeDaily(activity, range, allAttempts);
  ...
  const report = {
    ...
    summary: {
      ...
      correct,
      incorrect: allAttempts.length - correct,
      accuracy: pct(correct, allAttempts.length),
      help,
      helpRate: pct(help, allAttempts.length),
      activeDays: activityDays,
      activeSeconds,
      activeMinutes: Math.round(activeSeconds / 60),
      distinctSkills: new Set(allAttempts.map((a) => a.skillId).filter(Boolean)).size,
      assignmentsCompleted: assignments.filter((a) => a.progress && a.progress.completed).length,
      masteryAttempts: allAttempts.filter((a) => a.source === "mastery_challenge" || a.source === "mastery_review").length,
      dailyGoalsCompleted: Number(goals.completedScheduledDays || 0),
    ...
    notes: { detailedHistoryAvailableDays: ACTIVITY_RETENTION_DAYS, legacyDetailNote: "Detailed attempt rows are available for retained Phase 5A activity only; older dates use permanent summaries when available." },
  };
