...
function dailyPayload(rec, localDate, options) {
  ...
  return { dailyStatus, recommendations, masterySummary, assignments, learningPlan: LearningPlan.publicPlan(learningPlan), achievements };
}
...
function assignmentStatusPayload(rec, localDate) {
  ...
  return { assignments: extras.assignments, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls, { localDate }), learningPlan: extras.learningPlan, dailyStatus: extras.dailyStatus, achievements: extras.achievements, controls: publicControls(rec.controls) };
}
...
async function playerPost(request, env) {
  ...
  if (action === "report" || action === "save" || action === "placement_start" || action === "placement_progress" || action === "placement_complete" || action === "placement_skip" ||
    ...
    if (action === "report") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, data: rec.data || null, controls: publicControls(rec.controls || defaultControls("4")), masterySummary: extras.masterySummary, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, learningPlan: extras.learningPlan, assignments: extras.assignments, achievements: extras.achievements, updated: rec.updated });
    }
    ...
    if (action === "daily_status") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, masterySummary: extras.masterySummary, learningPlan: extras.learningPlan, assignments: extras.assignments, achievements: extras.achievements, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "learning_plan_status") {
      ...
      if (extras.error) return json({ ok: false, error: extras.error }, 400);
      await putRecord(store, name, rec);
      return json({ ok: true, learningPlan: extras.learningPlan, recommendations: extras.recommendations, achievements: extras.achievements, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "achievement_status") {
      ...
      if (extras.error) return json({ ok: false, error: extras.error }, 400);
      await putRecord(store, name, rec);
      return json({ ok: true, achievements: extras.achievements, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "assignment_answer") {
      ...
      const payload = assignmentStatusPayload(rec, effective.date);
      await putRecord(store, name, rec);
      return json({ ok: true, correct: result.correct, completed: !!result.completed, rewardCoins: paid, progress: result.progress, assignment: Assignments.summarizeAssignment(rec.controls.assignments[found.idx], { localDate: effective.date, includeNotes: false }), question: nextQuestion, assignments: payload.assignments, recommendations: payload.recommendations, learningPlan: payload.learningPlan, dailyStatus: payload.dailyStatus, achievements: payload.achievements, data: rec.data || null, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "assignment_active_time") {
      ...
      const payload = assignmentStatusPayload(rec, effective.date);
      await putRecord(store, name, rec);
      return json({ ok: true, duplicate: !!result.duplicate, completed: !!result.completed, rewardCoins: paid, progress: result.progress, assignment: Assignments.summarizeAssignment(rec.controls.assignments[found.idx], { localDate: effective.date, includeNotes: false }), assignments: payload.assignments, recommendations: payload.recommendations, learningPlan: payload.learningPlan, dailyStatus: payload.dailyStatus, achievements: payload.achievements, data: rec.data || null, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "placement_complete") {
      ...
      const achievements = achievementPayload(rec, { acknowledgeAchievements: true });
      await putRecord(store, name, rec);
      return json({ ok: true, placement: publicPlacement(rec.controls.placement), achievements });
    }
    ...
    if (action === "mastery_challenge_complete") {
      ...
      const achievements = achievementPayload(rec, { acknowledgeAchievements: true });
      await putRecord(store, name, rec);
      return json({ ok: true, result: result.result, skillMastery: publicSkillMastery(rec.controls.skillMastery), masterySummary: Mastery.summary(rec.data || null, rec.controls), learningPlan, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls), achievements });
    }
