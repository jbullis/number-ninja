...
function dailyPayload(rec, localDate) {
  ...
  return { dailyStatus, recommendations, masterySummary, assignments, learningPlan: LearningPlan.publicPlan(learningPlan) };
}
...
function assignmentStatusPayload(rec, localDate) {
  const extras = dailyPayload(rec, localDate);
  if (extras.error) return extras;
  return { assignments: extras.assignments, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls, { localDate }), learningPlan: extras.learningPlan, dailyStatus: extras.dailyStatus, controls: publicControls(rec.controls) };
}
...
async function playerPost(request, env) {
  ...
  if (action === "login") {
    ...
    if (type === "student") {
      rec.controls = rec.controls || defaultControls("4");
      studentExtras = dailyPayload(rec, body.localDate);
  ...
  if (action === "report" || action === "save" || action === "placement_start" || action === "placement_progress" || action === "placement_complete" || action === "placement_skip" ||
    ...
    if (action === "report") {
      rec.controls = rec.controls || defaultControls("4");
      Mastery.ensureControls(rec.controls);
      const extras = dailyPayload(rec, body.localDate);
      if (extras.error) return json({ ok: false, error: extras.error }, 400);
      await putRecord(store, name, rec);
      return json({ ok: true, data: rec.data || null, controls: publicControls(rec.controls || defaultControls("4")), masterySummary: extras.masterySummary, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, learningPlan: extras.learningPlan, assignments: extras.assignments, updated: rec.updated });
    }
    ...
    if (action === "daily_status") {
      ...
      const extras = dailyPayload(rec, effective.date);
      await putRecord(store, name, rec);
      return json({ ok: true, dailyStatus: extras.dailyStatus, recommendations: extras.recommendations, masterySummary: extras.masterySummary, learningPlan: extras.learningPlan, assignments: extras.assignments, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "learning_plan_status") {
      ...
      if (extras.error) return json({ ok: false, error: extras.error }, 400);
      await putRecord(store, name, rec);
      return json({ ok: true, learningPlan: extras.learningPlan, recommendations: extras.recommendations, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "daily_problem_complete") {
      ...
      await putRecord(store, name, rec);
      return json({ ok: true, duplicate: !!result.duplicate, dailyStatus: result.status, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls, { localDate: effective.date }), learningPlan, assignments: Assignments.activeSummaries(rec.controls.assignments, { localDate: effective.date, includeNotes: false }), controls: publicControls(rec.controls) });
    }
    ...
    if (action === "assignment_active_time") {
      ...
      const payload = assignmentStatusPayload(rec, effective.date);
      await putRecord(store, name, rec);
      return json({ ok: true, duplicate: !!result.duplicate, completed: !!result.completed, rewardCoins: paid, progress: result.progress, assignment: Assignments.summarizeAssignment(rec.controls.assignments[found.idx], { localDate: effective.date, includeNotes: false }), assignments: payload.assignments, recommendations: payload.recommendations, learningPlan: payload.learningPlan, dailyStatus: payload.dailyStatus, data: rec.data || null, controls: publicControls(rec.controls) });
    }
    ...
    if (action === "mastery_challenge_complete") {
      ...
      const learningPlan = refreshLearningPlan(rec);
      await putRecord(store, name, rec);
      return json({ ok: true, result: result.result, skillMastery: publicSkillMastery(rec.controls.skillMastery), masterySummary: Mastery.summary(rec.data || null, rec.controls), learningPlan, recommendations: Recommendations.primaryAndAlternates(rec.data || null, rec.controls) });
    }
